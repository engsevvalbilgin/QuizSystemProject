...
function TeacherEditQuizPage() {
    ...
    const addQuestion = () => {
        const newQuestion = {
            // id: undefined, // Yeni soru olduğu için ID yok
            questionSentence: '',
                ...
                <div className="p-4 md:p-6 bg-white rounded-lg shadow-md">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        {quiz.questions.map((q, qIndex) => (
                            <div key={q.id || qIndex} className="p-4 border border-gray-200 rounded-md space-y-3">
                                ...
                                {q.questionTypeId === 1 && ( // Çoktan Seçmeli
                                    <div className="space-y-2 mt-2">
                                        ...
                                        {q.options.map((opt, oIndex) => (
                                            <div key={opt.id || oIndex} className="flex items-center space-x-2">
                                                <input
                                                    type="text"
                                                    value={opt.text}
                                        ...
                                        {q.options.map((opt,oIndex) => errors.questions[qIndex]?.options?.[oIndex] && <p key={`err-${oIndex}`} className="text-red-600 text-xs">Seçenek {oIndex+1} metni zorunludur.</p> )}
